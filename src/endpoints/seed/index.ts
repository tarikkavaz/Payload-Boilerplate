...
export const seed = async ({
  ...
  const peripheralsCategory = await payload.create({
  ...
  const monitorsCategory = await payload.create({
    collection: 'product-categories',
    data: {
      title: 'Monitors', 
    },
  ...
  const keyboardsCategory = await payload.create({
    collection: 'product-categories',
    data: {
      title: 'Keyboards',
    },
  ...
  const product2Doc = await payload.create({
    collection: 'products',
    data: JSON.parse(
      JSON.stringify({ ...product2, categories: [monitorsCategory.id] })
        .replace(/"\{\{IMAGE_1\}\}"/g, String(image2ID))
        .replace(/"\{\{IMAGE_2\}\}"/g, String(image3ID))
        .replace(/"\{\{AUTHOR\}\}"/g, String(demoAuthorID)),
    ),
